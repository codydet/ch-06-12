...
function parseEquation(tok) {
	...
	while (true) {
		...
		if (next == "=") {
			...
			throw {message: "Sinal + ou = esperado", start: tok.position()};
		} else if (next == "+") {
			tok.consume("+");
			lhs.push(parseTerm(tok));
		} else
			throw {message: "+ esperado", start: tok.position()};
	}
	...
	while (true) {
		...
		if (next == null)
			...
			throw {message: "+ ou termino esperado", start: tok.position()};
	}
...
function parseTerm(tok) {
	...
	if (next != null && next == "^") {
		...
		next = tok.peek();
		if (next == null)
			throw {message: "Numero ou sinal esperado", start: tok.position()};
		else
		...
		if (next == "+")
			...
			throw {message: "Sinal esperado", start: tok.position()};
	...
	if (items.length == 0) {
		throw {message: "Termo invalido - vazio", start: startPosition, end: tok.position()};
	} else if (elems.indexOf("e") != -1) {  // If it's the special electron element
		if (items.length > 1)
			throw {message: "Termo invalido - elétron precisa permanecer sozinho", start: startPosition, end: tok.position()};
		else if (charge != 0 && charge != -1)
			throw {message: "Termo invalido - carga invalida do elétron", start: startPosition, end: tok.position()};
		// Tweak data
		items = [];
		charge = -1;
		...
		for (var i = 0; i < elems.length; i++) {
			if (/^[a-z]+$/.test(elems[i]))
				throw {message: 'Nome de elemento invalido "' + elems[i] + '"', start: startPosition, end: tok.position()};
		}
	}
...
function parseGroup(tok) {
	...
	while (true) {
		var next = tok.peek();
		if (next == null)
			throw {message: "Elemento, grupo, ou fechamento de parênteses esperado", start: tok.position()};
		else if (next == "(")
			items.push(parseGroup(tok));
		else if (/^[A-Za-z][a-z]*$/.test(next))
			items.push(parseElement(tok));
			...
			tok.consume(")");
			if (items.length == 0)
				throw {message: "Grupo vazio", start: startPosition, end: tok.position()};
			break;
		} else
			throw {message: "Elemento, grupo, ou fechamento de parênteses esperado", start: tok.position()};
	}
...
function Tokenizer(str) {
	...
	this.peek = function() {
		...
		if (match == null)
			throw {message: "Símbolo invalido", start: i};
		return match[0];
	};
...
//Me =D
function testeresposta(formulaStr) {
	// Clear output
	setMessage("");
	var balancedElem = document.getElementById("balanced");
	var codeOutElem  = document.getElementById("codeOutput");
